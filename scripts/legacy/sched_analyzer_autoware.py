                ...
                if cpu_info['cpu'+str(i)][j][PREV_COMM] == process_name[k]:
                    if cpu_info['cpu'+str(i)][j][PREV_PID] == per_cpu_start_info['cpu'+str(i)][process_name[k]][2]:
                        if per_cpu_start_info['cpu'+str(i)][process_name[k]][0]:
                            ...
                            if len(time_range) == 2 and process_info['StartTime'] < time_range[0] or process_info['EndTime'] > time_range[1]: break                            
...
if __name__ == "__main__":
    ...
    file = open('/home/hayeonp/git/ftrace_sched_analyzer/data/synthetic_task_log/221011_synthetic_chain_with_sched_instance.txt', 'r')

    # input: Dir of Autwoare csv logs - data/sample_autoware_log
    autoware_log_dir = '/home/hayeonp/git/Autoware_Analyzer/files/response_time'

    # input: e2e file - data/sample_autoware_log/system_instance.csv
    autoware_e2e_log_path = '/home/hayeonp/git/Autoware_Analyzer/files/response_time/system_instance.csv'

    per_cpu_info, process_name = parse_ftrace_log(file ,target_process_name)
    ...
    with open(file_path + '/data/220923_autoware_parsed_log2.json', 'w') as json_file:
